import {
  ...
  const HiddenAdminLoginPage = () => {
    const { classes } = useStyles();
  
    //Checking login data
    const login = async (values: { email: string; password: string }) => {
      AdminAPI.hiddenLogin(values)
        .then((response: any) => {
  
          // save user details in the local storage
          localStorage.setItem("user-secreteAdmin-session", JSON.stringify(response.data));
      ...
      <div className={classes.wrapper} style={{
        ...
        <Paper className={classes.form} radius={0} p={30}>
          <Title order={2} className={classes.title} ta="center" mt="md" mb={50}>
            Welcome back To the Secrete Portal
          </Title>
  ...
  export default HiddenAdminLoginPage;